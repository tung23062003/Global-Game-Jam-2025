namespace UIWidgets.Menu
{
	...
	public partial class ContextMenuView : MonoBehaviourInitiable, IPointerEnterHandler, IPointerExitHandler, IStylable
	{
		[SerializeField]
		...
		public void Subscribe(IMenuSubscriber menu) => Subscribers.Add(menu);

		/// <summary>
		/// Unsubscribe menu instance.
		/// </summary>
		/// <param name="menu">Menu.</param>
		public void Unsubscribe(IMenuSubscriber menu) => Subscribers.Remove(menu);
		...
		protected static void ResetItems(IMenuSubscriber subscriber) => subscriber.ResetItems();
		...
		/// </summary>
		/// <param name="subscriber">Subscriber.</param>
		protected static void UpdateItems(IMenuSubscriber subscriber) => subscriber.UpdateItems();
		...
		public virtual ContextMenuView Instance()
		{
			...
			else
			{
				...
				Utilities.FixInstantiated(EmptyTemplate.RectTransform, menu.RectTransform);
				menu.Init();
			}
		...
		protected virtual void CreateEmptyTemplate()
		{
			...
			Utilities.FixInstantiated(RectTransform, EmptyTemplate.RectTransform);
			...
			{
				if (!Utilities.IsNull(EmptyTemplate.defaultItemTemplate))
				{
