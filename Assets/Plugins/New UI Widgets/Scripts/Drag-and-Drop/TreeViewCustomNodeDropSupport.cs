namespace UIWidgets
{
		...
		where TTreeViewComponent : TreeViewComponentBase<TTreeViewItem>
	{
		...
		/// <summary>
		/// If enabled drop type (before, after, child) determined by pointer position..
		/// </summary>
		[SerializeField]
		[Tooltip("If enabled drop type (before, after, child) determined by pointer position.")]
		public bool AutoDropPosition = true;

		/// <summary>
		/// Drop position if AutoDropPosition disabled.
		/// </summary>
		[SerializeField]
		[EditorConditionBool(nameof(AutoDropPosition), false)]
		public TreeNodeDropType DropPosition = TreeNodeDropType.Child;
		...
		/// TreeView.
		/// </summary>
		protected TTreeView TreeView => Source.Owner as TTreeView;
		...
		public virtual void Drop(TreeNode<TTreeViewItem> data, PointerEventData eventData)
		{
			...
			using (var _ = TreeView.Nodes.BeginUpdate())
			{
