namespace UIWidgets
{
	...
	public class CalendarBase : DateBase
	{
		...
		{
			get => calendarDateTemplate;

			set
			{
				if (calendarDateTemplate != value)
				{
		...
		{
			get => calendarDayOfWeekTemplate;

			set
			{
				if (calendarDayOfWeekTemplate != value)
				{
		...
		public DateTime DateDisplay
		{
			...
			set
			{
				if (dateDisplay != value)
				{
					...
					OnDateDisplayChanged.Invoke(dateDisplay);
				}
			}
		}
		...
		{
			get => monthTextAdapter;

			set
			{
				if (monthTextAdapter != value)
				{
		...
		/// <summary>
		/// Event called when displayed date changed.
		/// </summary>
		[SerializeField]
		public CalendarDateEvent OnDateDisplayChanged = new CalendarDateEvent();
		...
		protected virtual void UpdateDays()
		{
			...
			var ev_go = EventSystem.current.gameObject;
			var ev_day = (ev_go != null) ? ev_go.GetComponent<CalendarDateBase>() : null;
			var selected = ev_day != null && Days.Contains(ev_day);
			...
			for (int i = 0; i < n; i++)
			{
				...
				if (selected && IsSameDay(Date, date) && (day.Selectable != null))
				{
					var go = day.gameObject;
					Updater.RunOnceNextFrame(go, () => EventSystem.current.SetSelectedGameObject(go));
				}
			}
		}
