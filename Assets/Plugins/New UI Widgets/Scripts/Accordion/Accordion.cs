namespace UIWidgets
{
	...
	public class Accordion : UIBehaviourInteractable, IStylable
	{
		/// <summary>
		...
		{
			get => new List<AccordionItem>(DataSource);

			set => new ObservableList<AccordionItem>(value);
		}
		...
		/// </summary>
		[SerializeField]
		[EditorConditionBool(nameof(Animate))]
		...
		protected virtual void AddCallback(AccordionItem item)
		{
			...
			component.Item = item;
			component.OnItemClick.AddListener(ProcessToggleItem);
		...
		protected virtual void RemoveCallback(AccordionItemComponent component)
		{
			...
			component.Item = null;
			component.OnItemClick.RemoveListener(ProcessToggleItem);
		}
		...
		/// </summary>
		/// <param name="item">Item.</param>
		protected virtual void ProcessToggleItem(AccordionItem item)
		{
			if (IsInteractable())
			{
				ToggleItem(item);
		...
		protected void UpdateItemSize(AccordionItem item)
		{
			...
			var size = new LayoutElementData(item.ContentObjectRect);
