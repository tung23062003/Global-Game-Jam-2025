namespace UIWidgets
{
	...
	public class FileDialog : Picker<string, FileDialog>, IUpgradeable
	{
		...
		protected override void AddListeners()
		{
			...
			FileListView.OnSelectInternal.AddListener(FileSelected);
		...
		protected override void RemoveListeners()
		{
			...
			FileListView.OnSelectInternal.RemoveListener(FileSelected);
		...
		public async void OkClick()
		{
			...
			if (RequestConfirmationIfFileExists && File.Exists(fullname))
			{
				...
				var result = await confirm.ShowAsync(false);
				if (result)
				{
					Selected(fullname);
				}
