namespace UIWidgets
{
	...
	public class Combobox : MonoBehaviourInitiable, ISubmitHandler, IStylable, IUpgradeable
	{
		[SerializeField]
		ListView listView;
		...
		protected virtual void SetListView(ListView value)
		{
			if (listView != null)
			{
				...
				listView.InstancesEventsInternal.Cancel.RemoveListener(OnItemListViewCancel);
			...
			if (listView != null)
			{
				...
				listView.InstancesEventsInternal.Cancel.AddListener(OnItemListViewCancel);
		...
		{
			if (TryGetComponent<Image>(out var input_bg))
			{
				style.Combobox.SingleInputBackground.ApplyTo(input_bg);
			}
		...
		public virtual bool GetStyle(Style style)
		{
			if (TryGetComponent<Image>(out var input_bg))
			{
				style.Combobox.SingleInputBackground.GetFrom(input_bg);
			}
