namespace UIWidgets
{
	...
	public class ObservableList<T> : IObservable, IObservableList<T>, IList<T>, IReadOnlyList<T>
	{
		/// <summary>
		...
		/// <summary>
		/// Occurs when data changed.
		/// Should be used with MonoBehaviour methods.
		/// Reason:
		/// - Listeners can be added outside Start() or OnEnable() methods
		/// - Listeners often removed inside OnDestroy() method
		/// - OnDestroy() called only if game object was activated
		...
		/// Use stable sort.
		/// </summary>
		public bool UseStableSort = true;
		...
		public ObservableList(bool observeItems, int capacity = 0)
		{
			...
			OnChange += OnChangeMono.Invoke;
			OnCollectionChange += OnCollectionChangeMono.Invoke;
			OnCollectionItemChange += OnCollectionItemChangeMono.Invoke;
		...
		/// <param name="useSameList">Use same list instead of a new one.</param>
		public ObservableList(List<T> list, bool observeItems, bool useSameList = false)
		{
			ObserveItems = observeItems;
			...
			Items = useSameList ? list : new List<T>(list);
		...
		/// <param name="list">list.</param>
		/// <param name="observeItems">Is need to observe items? If true ObservableList.OnChange will be raised on item OnChange or PropertyChanged.</param>
		public ObservableList(List<T> list, bool observeItems = true)
		{
			ObserveItems = observeItems;
		...
		void CollectionChanged(bool resort)
		{
			...
			else
			{
				...
				{
					StableSort.Sort(Items, comparison);
				}
		...
		void CollectionItemChanged(string property)
		{
			...
			else
			{
				...
				OnCollectionItemChange?.Invoke();

				OnChange?.Invoke();

				PropertyChanged?.Invoke(this, new PropertyChangedEventArgs(property ?? "Collection"));
			}
		}
		...
		/// <example>using var _ = list.BeginUpdate();</example>
		public ListUpdater BeginUpdate()
		{
		...
		public T this[int index]
		{
			...
			{
				var old = Items[index];
				if (!IsValueType && ReferenceEquals(old, value))
				{
					return;
				}
		...
		public ObservableList<TOutput> ConvertAll<TOutput>(Converter<T, TOutput> converter, bool observeItems = true) => Convert<TOutput>(converter, observeItems);
		...
		public ObservableList<T> FindAll(Predicate<T> match, bool observeItems = true) => new ObservableList<T>(Items.FindAll(match), observeItems);
		...
		public ObservableList<T> GetRange(int index, int count, bool observeItems = true) => new ObservableList<T>(Items.GetRange(index, count), observeItems);
		...
		/// </summary>
		public void Sort() => Sort(UseStableSort);

		/// <summary>
		/// Sorts the elements in the entire ObservableList{T} using the default comparer.
		/// </summary>
		/// <param name="stable">Use stable sort.</param>
		public void Sort(bool stable)
		{
			if (stable)
			{
				StableSort.Sort(Items, Comparer<T>.Default.Compare);
			}
			else
			{
				Items.Sort();
		...
		public void Sort(Comparison<T> sortComparison) => Sort(sortComparison, UseStableSort);
		...
		/// <param name="sortComparison">The System.Comparison{T} to use when comparing elements.</param>
		/// <param name="stable">Use stable sort.</param>
		public void Sort(Comparison<T> sortComparison, bool stable)
		{
			if (stable)
			{
				StableSort.Sort(Items, sortComparison);
			}
			else
			{
		...
		/// </summary>
		/// <param name="comparer">The IComparer{T} implementation to use when comparing elements, or null to use the default comparer Comparer{T}.Default.</param>
		public void Sort(IComparer<T> comparer) => Sort(comparer, UseStableSort);
		...
		public void Sort(IComparer<T> comparer, bool stable)
		{
			if (stable)
			{
				...
				StableSort.Sort(Items, comparer.Compare);
			}
			else
			{
				Items.Sort(comparer);
		...
		/// <param name="comparer">The IComparer{T} implementation to use when comparing elements, or null to use the default comparer Comparer{T}.Default.</param>
		public void Sort(int index, int count, IComparer<T> comparer) => Sort(index, count, comparer, UseStableSort);
		...
		/// <param name="comparer">The IComparer{T} implementation to use when comparing elements, or null to use the default comparer Comparer{T}.Default.</param>
		/// <param name="stable">Use stable sort.</param>
		public void Sort(int index, int count, IComparer<T> comparer, bool stable)
		{
			if (stable)
			{
				comparer ??= Comparer<T>.Default;
				StableSort.Sort(Items, index, count, comparer.Compare);
			}
			else
			{
		...
		/// </summary>
		/// <returns>List.</returns>
		public List<T> ListReference() => Items;
	}
}