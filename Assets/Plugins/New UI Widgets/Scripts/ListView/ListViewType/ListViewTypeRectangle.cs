namespace UIWidgets
{
		...
		where TItemView : ListViewItem
	{
		...
		protected abstract class ListViewTypeRectangle : ListViewTypeBase
		{
			...
			/// <inheritdoc/>
			public override bool CanScroll => Owner.Viewport.ScaledAxisSize < ListSize();
			...
			public override bool IsRequiredCenterTheItems()
			{
				...
				return Owner.Viewport.ScaledAxisSize > ListSize();
			}
			...
			public override float ValidateScrollPosition(float position)
			{
				...
				return Mathf.Clamp(position, 0, ListSize() - Owner.Viewport.ScaledAxisSize);
			}

			/// <inheritdoc/>
			public override float CenteredFillerSize()
			{
				return (Owner.Viewport.ScaledAxisSize - ListSize() - Owner.LayoutBridge.GetFullMargin()) / 2f;
			}
			...
			public override float ValidatePosition(float position)
			{
				...
				var list_size = ListSize() + Owner.LayoutBridge.GetSpacing();
				var half_viewport = Owner.Viewport.ScaledAxisSize / 2f;
				if (Owner.IsHorizontal())
				{
					...
					else if (position > half_viewport)
					{
						position -= list_size;
					}
				}
			...
			{
				var result = position ?? Owner.ContainerAnchoredPosition;
			...
			{
				var pos = GetPositionVector(position);
			...
			{
				return GetPosition() + (Owner.Viewport.ScaledAxisSize / 2f) - Owner.LayoutBridge.GetMargin();
			}
			...
			public override void UpdateLayout(bool recalculate = true)
			{
				...
				else
				{
					...
					if (Owner.ReversedOrderRequired)
					{
						...
						var scroll_size = Owner.Viewport.ScaledAxisSize - Owner.LayoutBridge.GetFullMargin();
				...
				// IMPORTANT: layout should be updated immediately to update items sizes and container size
				// otherwise scroll position will be corrected on next frame
				// or in this frame but will cause inertia animation when corrected scroll position is out of border
