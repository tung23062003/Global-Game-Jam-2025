namespace UIWidgets
{
	...
	public class EasyLayoutBridge : ILayoutBridge
	{
		...
		/// </summary>
		protected bool ControlRectTransform;
		...
		public EasyLayoutBridge(EasyLayout layout, RectTransform defaultItem, bool updateContentSizeFitter = true, bool controlRectTransform = true)
		{
			...
			ControlRectTransform = controlRectTransform;
		...
		public Vector2 GetItemSize()
		{
			...
			var layout_size = new LayoutElementData(DefaultItem);
			var width = Mathf.Max(rect.width, layout_size.PreferredWidth);
			var height = Mathf.Max(rect.height, layout_size.PreferredHeight);
		...
		public void ResetMargin()
		{
			if (Layout.Symmetric)
			{
				Layout.Margin = IsHorizontal
					? new Vector2(0f, Layout.Margin.y)
					: new Vector2(Layout.Margin.x, 0f);
			}
			else
			{
				if (IsHorizontal)
