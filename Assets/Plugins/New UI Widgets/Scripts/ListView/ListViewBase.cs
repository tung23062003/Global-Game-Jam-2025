...
namespace UIWidgets
{
			...
			IStylable, IUpgradeable, UIThemes.ITargetOwner
	{
		...
		{
			get => new List<ListViewItem>(instances);

			set => UpdateComponents(value);
		}
		...
		/// <summary>
		/// MultipleSelect should work with a click with the pressed Shift or Ctrl key.
		/// </summary>
		[SerializeField]
		[Tooltip("MultipleSelect should work with a click with the pressed Shift or Ctrl key.")]
		public bool MultipleSelectRequireKeys = false;
		...
		{
			get => highlightedBackgroundColor;
		...
		{
			get => defaultEvenBackgroundColor;
		...
		public virtual bool LoopedListAvailable => false;

		/// <summary>
		/// Gets the first displayed index.
		/// </summary>
		/// <value>The first displayed index.</value>
		public virtual int DisplayedIndexFirst => 0;

		/// <summary>
		/// Gets the last displayed index.
		/// </summary>
		/// <value>The last displayed index.</value>
		public virtual int DisplayedIndexLast => instances.Count - 1;
		...
		/// </summary>
		[SerializeField]
		public bool LoopedNavigation = true;

		/// <summary>
		/// Number of visible items from current to border on navigation.
		/// </summary>
		[SerializeField]
		[Range(0, 5)]
		public int RequireVisible = 1;

		/// <summary>
		/// Stretches ListView to fully display the largest item.
		/// Works only with ListType = ListViewWithVariableSize.
		/// </summary>
		[SerializeField]
		[Tooltip("Stretches ListView to fully display the largest item.")]
		public bool StretchToMaxItemSize = false;
		...
		{
			get => ContainerRealAnchoredPosition;

			set => ContainerRealAnchoredPosition = value;
		}

		/// <summary>
		/// Real Container.anchoredPosition wrapper.
		/// </summary>
		protected virtual Vector2 ContainerRealAnchoredPosition
		{
			get
			{
		...
		public ListViewItemEvents ItemsEventsInternal => InstancesEventsInternal;
		...
		public ListViewItemEvents ItemsEvents => InstancesEvents;
		...
		{
			get => disabledContainer;

			protected set => disabledContainer = value;
		}
		...
		/// </summary>
		[DomainReloadExclude]
		public static readonly Func<int, bool> AlwaysAllow = (index) => true;
		...
		protected override void InitOnce()
		{
			...
			if (Navigation && !TryGetComponent<Selectable>(out var _))
			{
				gameObject.AddComponent<Selectable>();
				Debug.Log("The enabled Navigation option requires a Selectable component. It was added automatically.", this);
			}
		...
		/// </summary>
		[SerializeField]
		[Tooltip("Keep item highlight on pointer enter until will be selected another game object.")]
		public bool KeepHighlight = true;

		/// <summary>
		/// Prevent two highlighted game objects at same time.
		/// First highlighted object on navigation (selected by EventSystem).
		/// Second highlighted object on pointer enter.
		/// </summary>
		[SerializeField]
		[Tooltip("Prevent two highlighted game objects at same time.")]
		...
		protected virtual ListViewItem FindItemInHierarchy(GameObject go)
		{
			...
			current_transform.TryGetComponent<ListViewItem>(out var instance);
			return instance;
		}
		#endregion
		...
		{
			if (TryGetComponent<Selectable>(out var _))
			{
		...
		public virtual void Select(int index, bool raiseEvents)
		{
			...
			if (!IsValid(index))
			{
				var message = string.Format("Index must be between 0 and Items.Count ({0}), but {2}, Game object {1}.", (instances.Count - 1).ToString(), name, index.ToString());
		...
		/// <summary>
		/// Toggle index when MultiSelect is enabled.
		/// </summary>
		/// <param name="index">Index.</param>
		protected virtual void MultiSelectToggle(int index)
		{
			if (CompatibilityInput.ShiftPressed)
			{
				var have_selected = selectedIndices.Count > 0;
				var last_selected = selectedIndex;
				if (have_selected && last_selected != index)
				{
			...
			if (CompatibilityInput.CtrlPressed)
			{
				Toggle(index);
				return;
			}
			
			if (MultipleSelectRequireKeys)
			{
				DeselectAll();
				Select(index);
				return;
		...
		/// <param name="eventData">Event data.</param>
		/// <param name="index">Index.</param>
		/// <param name="forward">Navigate forward if true; otherwise backward.</param>
		/// <returns>true if navigation successful; otherwise false.</returns>
		protected virtual bool Navigate(AxisEventData eventData, int index, bool forward)
		{
			if (!IsValid(index))
			{
				return false;
			...
			var scroll_to = index + (forward ? RequireVisible : -RequireVisible);
			ScrollTo(scroll_to);
