namespace UIWidgets
{
	...
	public class ListView : ListViewBase, IStylable, IUpgradeable, IUpdatable, IListViewCallbacks<ListViewStringComponent>
	{
		...
		{
			get => new List<string>(DataSource);

			set => DataSource = new ObservableList<string>(value);
		}
...
#pragma warning restore 0612,0618
		...
		protected int DisplayedIndicesFirst => DisplayedIndices.Count > 0 ? DisplayedIndices[0] : -1;

		/// <summary>
		/// Gets the last displayed index.
		/// </summary>
		/// <value>The last displayed index.</value>
		protected int DisplayedIndicesLast => DisplayedIndices.Count > 0 ? DisplayedIndices[DisplayedIndices.Count - 1] : -1;
		...
		public bool Sort
		{
			...
			{
				sort = value;
				if (Sort && IsInited && sortFunc != null)
				{
					UpdateItems();
				}
			}
		...
		/// OnSelect event.
		/// </summary>
		[DataBindEvent(nameof(SelectedItem), nameof(SelectedItems))]
		...
		public ScrollRect ScrollRect
		{
			...
			set
			{
				if (scrollRect != null)
				{
					if (scrollRect.TryGetComponent<ResizeListener>(out var r))
					{
						r.OnResizeNextFrame.RemoveListener(SetNeedResize);
					}
		...
		public EasyLayout Layout => layout as EasyLayout;
		...
		public override bool LoopedListAvailable => LoopedList && CanOptimize() && (GetScrollSize() < ListSize());
		...
		protected virtual ListViewStringComponent Index2Template(int index) => DefaultItem;
		...
		{
			return scrollRect != null && (layout != null || Container.TryGetComponent<EasyLayout>(out var _));
		}
		...
		protected virtual void SetNewItems(ObservableList<string> newItems, bool updateView = true)
		{
			lock (DataSource)
			{
				...
				if (Unique)
				{
					using var __ = newItems.BeginUpdate();
