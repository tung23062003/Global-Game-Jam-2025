namespace UIWidgets
{
	...
	public class DialogButtonCustom<TOwner, TOwnerBase, TButtonConfig>
		where TOwner : class, IHideable, TOwnerBase, ILocalizationSupport
		where TButtonConfig : ButtonConfiguration<TOwnerBase>
	{
		...
		protected void CreateButton(Button template)
		{
			...
			if (Button.TryGetComponent<DialogButtonComponentBase>(out var temp))
			{
				ButtonComponent = temp;
			}
		...
		public virtual void UpdateButtonName()
		{
			...
			{
				ButtonComponent.SetButtonName(Owner.LocalizationSupport ? Localization.GetTranslation(Info.Label) : Info.Label);
			}
		}
