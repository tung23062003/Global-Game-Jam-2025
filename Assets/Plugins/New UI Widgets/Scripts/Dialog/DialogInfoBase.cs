namespace UIWidgets
{
	...
	{
		[SerializeField]
		[Tooltip("If enabled translates dialog info using Localization.GetTranslation().")]
		[FormerlySerializedAs("LocalizationSupport")]
		...
		public virtual void SetInfo(string title, object[] titleArgs, string message, object[] messageArgs, Sprite icon)
		{
			...
			CurrentTitleArgs = titleArgs ?? Compatibility.EmptyArray<object>();
			...
			CurrentMessageArgs = messageArgs ?? Compatibility.EmptyArray<object>();
