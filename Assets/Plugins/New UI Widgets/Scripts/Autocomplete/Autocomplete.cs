namespace UIWidgets
{
	...
	public class Autocomplete : MonoBehaviourInitiable, IStylable, IUpgradeable, IUpdatable, ILateUpdatable
	{
		/// <summary>
		...
		/// Allow to work with default InputField and TMPro InputField.
		/// </summary>
		public virtual InputFieldAdapter InputFieldAdapter => inputFieldAdapter;
		...
		public string Value
		{
			get => InputFieldAdapter.Value;

			set => InputFieldAdapter.Value = value;
		}
		...
		public virtual bool Startswith(string value) => UtilitiesCompare.StartsWith(value, Query, CaseSensitive);
		...
		public virtual bool Contains(string value) => UtilitiesCompare.Contains(value, Query, CaseSensitive);
		...
		/// <param name="value">Value.</param>
		protected virtual string GetStringValue(string value) => value;
		...
		protected virtual void ShowOptions()
		{
			...
			if (canvas != null)
			{
				...
				var resize = Utilities.RequireComponent<ResizeListener>(canvas);
				CanvasResize.Clear();
		...
		protected virtual void OnDestroy()
		{
			...
			if (InputFieldAdapter != null)
			{
				...
				if (InputFieldAdapter.TryGetComponent<InputFieldListener>(out var inputListener))
				{
