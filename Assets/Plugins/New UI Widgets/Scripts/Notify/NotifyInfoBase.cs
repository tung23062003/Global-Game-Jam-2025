namespace UIWidgets
{
	...
	{
		[SerializeField]
		[Tooltip("If enabled translates notification using Localization.GetTranslation().")]
		...
		protected virtual void UpdateText()
		{
			...
			{
				var message = (!string.IsNullOrEmpty(CurrentMessage) && LocalizationSupport)
					? Localization.GetTranslation(CurrentMessage)
					: CurrentMessage;
		...
		public virtual void SetInfo(string message, params object[] args)
		{
			...
			CurrentMessageArgs = args ?? Compatibility.EmptyArray<object>();
