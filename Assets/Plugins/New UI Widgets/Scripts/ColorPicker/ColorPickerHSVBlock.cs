namespace UIWidgets
{
	...
	public class ColorPickerHSVBlock : ColorPickerBlock
	{
		[SerializeField]
		Slider hSlider;
		...
		{
			if ((hSlider != null) && (hSlider.handleRect != null) && hSlider.handleRect.TryGetComponent<Image>(out var h_img))
			{
				...
				handle_style.ApplyTo(h_img);
			}

			if ((sSlider != null) && (sSlider.handleRect != null) && sSlider.handleRect.TryGetComponent<Image>(out var s_img))
			{
				var handle_style = UtilitiesUI.IsHorizontal(sSlider)
					? styleColorPicker.SliderHorizontalHandle
					: styleColorPicker.SliderVerticalHandle;
				handle_style.ApplyTo(s_img);
			}

			if ((vSlider != null) && (vSlider.handleRect != null) && vSlider.handleRect.TryGetComponent<Image>(out var v_img))
			{
				var handle_style = UtilitiesUI.IsHorizontal(vSlider)
					? styleColorPicker.SliderHorizontalHandle
					: styleColorPicker.SliderVerticalHandle;
				handle_style.ApplyTo(v_img);
			}
