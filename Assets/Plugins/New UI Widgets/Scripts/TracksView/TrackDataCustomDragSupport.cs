namespace UIWidgets
{
		...
		where TPoint : IComparable<TPoint>
	{
		...
		protected override void InitDrag(PointerEventData eventData)
		{
			...
			TryGetComponent<TDataView>(out var view);
			Data = view.Data;
			Data.IsDragged = true;
		...
		{
			if ((Owner != null) && !Utilities.IsNull(CurrentTarget))
			{
...
#endif
				{
					if ((Owner != null) && Owner.AllowDragOutside && Utilities.IsNull(CurrentTarget))
					{
		...
		protected virtual void ShowDragInfo()
		{
			...
			DragInfo.RectTransform.SetParent(DragPoint, false);
			DragInfo.RectTransform.anchoredPosition = DragInfoOffset;

			DragInfo.SetData(null, Data);
			Owner.SetDataSizeAndPosition(DragPoint, Data, 0f);
