namespace UIWidgets
{
		...
		where TTrackForm : TrackFormBase<TData, TPoint>
	{
		...
		protected ListComponentPool<TDataView> DataViews
		{
			get
			{
				if ((dataViews == null) || (dataViews.Template == null))
				{
		...
		protected ListComponentPool<TTrackView> TracksHeaders
		{
			get
			{
				if ((tracksHeaders == null) || (tracksHeaders.Template == null))
				{
		...
		protected ListComponentPool<TTrackBackground> TracksBackgrounds
		{
			get
			{
				if ((tracksBackgrounds == null) || (tracksBackgrounds.Template == null))
				{
		...
		protected override void UpdateDataView()
		{
			...
			foreach (var item in VisibleDataViews)
			{
				SetDataSizeAndPosition(item);
			}
		...
		protected virtual void SetDataSizeAndPosition(TracksDataViews.VisibleItem item)
		{
			SetDataSizeAndPosition(item.View.RectTransform, item.Data, item.Height);
		}
		...
		{
			foreach (var v_item in VisibleDataViews)
			{
				SetDataPosition(v_item.View.RectTransform, v_item.Data);
			}

			UpdateBackgroundsPositions();
		}

		/// <summary>
		/// Update tracks backgrounds positions.
		/// </summary>
		protected virtual void UpdateBackgroundsPositions()
		{
			...
			for (int i = 0; i < Tracks.Count; i++)
			{
				...
				var bg_pos = position;
				bg_pos.y *= -1;
				SetDataPosition(bg.RectTransform, bg_pos);
