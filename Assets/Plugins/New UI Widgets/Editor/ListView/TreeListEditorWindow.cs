...
namespace UIWidgets
{
	...
	public abstract class TreeListEditorWindow : EditorWindow
	{
		...
		protected class Field
		{
			...
			{
				Property = Utilities.IsNull(item) ? null : item.FindPropertyRelative(name);
				Name = name;
				Header = header ?? name;
				Label = label ?? name;
			}
		...
		/// <summary>
		/// Serialized component of the target.
		/// </summary>
		protected abstract SerializedObject TargetSerialized
		{
			get;
		}
		...
		/// <summary>
		/// Gets serialized component of target.
		/// </summary>
		/// <typeparam name="TComponent">Type of component to serialize.</typeparam>
		/// <returns>Serialized component of target</returns>
		protected SerializedObject GetTargetSerialized<TComponent>()
			where TComponent : MonoBehaviour
		{
			...
			if (Utilities.IsNull(targetSerialized))
			{
				targetSerialized = new SerializedObject(component);
			}
		...
		{
			if (Utilities.IsNull(LabelStyle))
			{
			...
			UtilitiesEditor.ApplyModifiedProperties(TargetSerialized);
		}
