...
namespace UIWidgets
{
	...
	public class TreeViewDataSourceWindow : TreeListEditorWindow
	{
		...
		/// <inheritdoc/>
		protected override SerializedObject TargetSerialized
		{
			get
			{
				return GetTargetSerialized<TreeViewDataSource>();
			}
		}
		...
		{
			var window = GetWindow<TreeViewDataSourceWindow>();
			window.Show();
		}
		...
		protected override bool InitGUI()
		{
			...
			{
				if (Selection.activeGameObject.TryGetComponent<TreeViewDataSource>(out var component))
				{
			...
			List = TargetSerialized.FindProperty("Data");
