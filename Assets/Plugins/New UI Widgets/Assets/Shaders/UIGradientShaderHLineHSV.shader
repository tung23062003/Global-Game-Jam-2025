...
// Display the two colors interpolation between HSV colors from the left side to the right side.
...
Shader "Custom/New UI Widgets/GradientShaderHLineHSV"
{
	Properties
	{
		// Sprite texture
	...
	SubShader
	{
		...
		CGPROGRAM
			...
			fixed4 frag(v2f IN) : SV_Target
			{
				...
				color = color * HSVtoRGB(lerp(_ColorLeft, _ColorRight, IN.texcoord.x));
				#else
				color = color * HSVtoRGB(lerp(LinearToGammaSpace4(_ColorLeft), LinearToGammaSpace4(_ColorRight), IN.texcoord.x));
