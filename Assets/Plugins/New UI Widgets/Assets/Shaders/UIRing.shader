...
// Ring shader.
// Should be used only "_RingColor", "_Thickness", "_Padding", "_Resolution", "_Fill", and "_Transparent" shader properties,
	...
	SubShader
	{
		...
		CGPROGRAM
			...
			fixed4 frag(v2f IN) : SV_Target
			{
				...
				float4 ring_color = lerp(color, _RingColor, _Fill);
				color.rgb = lerp(color.rgb, ring_color.rgb, _Fill && _Transparent);
				color.a = lerp(color.a, 0, _Fill && _Transparent);

				float quality = 0.01 / _Resolution;
				float outer_radius = 0.5 - ((_Padding + 0.01) / _Resolution);
				float inner_radius = max(0, min(outer_radius - (_Thickness / _Resolution), outer_radius - (0.01 / _Resolution)));
				...
				float4 temp_color = lerp(color, ring_color, outer_alpha);
				color = lerp(temp_color, color, inner_alpha);
				color.a = lerp(color.a * (outer_alpha - inner_alpha), color.a, _Fill);
