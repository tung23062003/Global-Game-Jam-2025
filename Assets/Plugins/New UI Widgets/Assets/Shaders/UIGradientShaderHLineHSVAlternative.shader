...
// Display the two colors HSV interpolation between RGB colors from the left side to the right side.
...
Shader "Custom/New UI Widgets/GradientShaderHLineHSVAlternative"
{
	Properties
	{
		// Sprite texture
		...
		_ColorLeft ("Color Left", Color) = (0,1,1,1)
	...
	SubShader
	{
		...
		CGPROGRAM
			...
			fixed4 frag(v2f IN) : SV_Target
			{
				...
				color = color * lerpHSVAlternative(_ColorLeft, _ColorRight, IN.texcoord.x);
				#else
				color = color * lerpHSVAlternative(LinearToGammaSpace4(_ColorLeft), LinearToGammaSpace4(_ColorRight), IN.texcoord.x);
