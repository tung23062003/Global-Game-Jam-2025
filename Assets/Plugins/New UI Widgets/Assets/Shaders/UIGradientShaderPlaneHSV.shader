...
Shader "Custom/New UI Widgets/GradientShaderPlaneHSV"
{
	Properties
	{
		// Sprite texture
	...
	SubShader
	{
		...
		CGPROGRAM
			...
			fixed4 frag(v2f IN) : SV_Target
			{
				...
				color = color * HSVtoRGB(lerp(_ColorLeft, _ColorRight, IN.texcoord.x) + lerp(_ColorBottom, _ColorTop, IN.texcoord.y));
				#else
				color = color * HSVtoRGB(lerp(LinearToGammaSpace4(_ColorLeft), LinearToGammaSpace4(_ColorRight), IN.texcoord.x) + lerp(LinearToGammaSpace4(_ColorBottom), LinearToGammaSpace4(_ColorTop), IN.texcoord.y));
