...
// Display the four colors interpolation between RGB colors from the side.
// _ColorLeft - RGB color on the left side.
// _ColorRight - RGB color on the right side.
// _ColorBottom - RGB color on the bottom side.
// _ColorTop - RGB color on the bottom side.
...
Shader "Custom/New UI Widgets/GradientShaderPlaneRGB"
{
	Properties
	{
		// Sprite texture
	...
	SubShader
	{
		...
		CGPROGRAM
			...
			fixed4 frag(v2f IN) : SV_Target
			{
				...
				color = color * (lerp(_ColorLeft, _ColorRight, IN.texcoord.x) + lerp(_ColorBottom, _ColorTop, IN.texcoord.y));
				#else
				color = color * (lerp(LinearToGammaSpace4(_ColorLeft), LinearToGammaSpace4(_ColorRight), IN.texcoord.x) + lerp(LinearToGammaSpace4(_ColorBottom), LinearToGammaSpace4(_ColorTop), IN.texcoord.y));
