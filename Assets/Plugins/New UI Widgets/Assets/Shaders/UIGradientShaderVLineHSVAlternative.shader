...
// Display the two colors HSV interpolation between RGB colors from the bottom side to the top side.
...
Shader "Custom/New UI Widgets/GradientShaderVLineHSVAlternative"
{
	Properties
	{
		// Sprite texture
	...
	SubShader
	{
		...
		CGPROGRAM
			...
			fixed4 frag(v2f IN) : SV_Target
			{
				...
				color = color * lerpHSVAlternative(_ColorBottom, _ColorTop, IN.texcoord.y);
				#else
				color = color * lerpHSVAlternative(LinearToGammaSpace4(_ColorBottom), LinearToGammaSpace4(_ColorTop), IN.texcoord.y);
