...
Shader "Custom/New UI Widgets/UIFlareGlobal"
{
	Properties
	{
		...
		// flare height delay rate, range -1..1
		_FlareHeightDelay("Flare Height Delay", Float) = 0.0
	...
	SubShader
	{
		...
		CGPROGRAM
			...
			fixed4 frag(v2f IN) : SV_Target
			{
				...
				float y = (IN.screenPos.y + 1) / 2;

				float delay = 1.0 + (_FlareDelay * _FlareSpeed);
				float flare_center = frac(_Time.y * (_FlareSpeed / delay)) * delay + y * _FlareHeightDelay;
				float rate = flare_distance(flare_center, _FlareSize, x, delay);

				float4 color = (UNITY_SAMPLE_SCREENSPACE_TEXTURE(_MainTex, IN.texcoord) + _TextureSampleAdd) * IN.color;
