...
Shader "Custom/New UI Widgets/GradientShaderHLineHSVDirect"
{
	Properties
	{
		// Sprite texture
	...
	SubShader
	{
		...
		CGPROGRAM
			...
			fixed4 frag(v2f IN) : SV_Target
			{
				...
				color = color * lerpHSV(_ColorLeft, _ColorRight, IN.texcoord.x);
				#else
				color = color * lerpHSV(LinearToGammaSpace4(_ColorLeft), LinearToGammaSpace4(_ColorRight), IN.texcoord.x);
