...
// Display the two colors interpolation between HSV colors from the bottom side to the top side.
...
Shader "Custom/New UI Widgets/GradientShaderVLineHSV"
{
	Properties
	{
		// Sprite texture
	...
	SubShader
	{
		...
		CGPROGRAM
			...
			fixed4 frag(v2f IN) : SV_Target
			{
				...
				color = color * HSVtoRGB(lerp(_ColorBottom, _ColorTop, IN.texcoord.y));
				#else
				color = color * HSVtoRGB(lerp(LinearToGammaSpace4(_ColorBottom), LinearToGammaSpace4(_ColorTop), IN.texcoord.y));
