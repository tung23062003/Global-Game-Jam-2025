	...
	public class TestTreeView : MonoBehaviour
	{
		...
		public void ProcessSelectedNodes()
		{
			foreach (var node in Tree.SelectedNodes)
			{
				...
				var instance = Tree.GetItemInstance(node.Index);

				// not visible instance will be null
				if (instance != null)
				{
		...
		{
			Nodes.OnChangeMono.AddListener(UpdateUINodes);
		...
		{
			Nodes?.OnChangeMono.RemoveListener(UpdateUINodes);
		}
		...
		public void AddSubNodes()
		{
			...
			using var _ = node.Nodes.BeginUpdate();
		...
		{
			Nodes.Filter(node => UtilitiesCompare.Contains(node.Item.Name, nameContains));
		}
		...
		public static ObservableList<TreeNode<TreeViewItem>> GenerateTreeNodes(IList<int> items, string nameStartsWith = "Node ", bool isExpanded = true, Sprite icon = null, int start = 0)
		{
